...
def printTable(table):
	sizes = []
	#descobrir a maior palavra de cada coluna
	for y in range(len(table)):
		sizes.append(0)
		for x in range(len(table[y])):
			if len(table[y][x]) > sizes[y]:
				sizes[y] = len(table[y][x])
	#imprimir na formatação correta			
	for i in range(len(table[0])):
		draw = ''
		for j in range(len(table)):
			draw = draw + table[j][i].rjust((sizes[j] + 2), ' ')
		print(draw)
